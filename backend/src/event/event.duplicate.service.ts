...
import {
  BookingFormProcessor,
  DatePeriod,
} from '../booking/util/booking-form-processor';
...
export class EventCreationService {
  ...
  public async findEventDuplicateOrCreateNew(
    ...
    for (let promoterEvent of promoterEvents) {
      const nameDuplicated = this.isEventNameDuplicated(
        newEventName,
        promoterEvent.name,
      );
      if (nameDuplicated) {
        this.logger.log(
          `Found event with similar name: ${newEventName} related to promoter: ${ctx.booking.promoterUid}`,
        );
  ...
  private async createNewEvent(
    ...
    await event.save();
    this.logger.log(
      `Created new event ${newEventName}, related to promoter: ${booking.promoterUid}`,
    );
    return event;
  }
  ...
  private isEventNameDuplicated(
    newEventName: string,
    promoterEventName: string,
    threshold = 5,
  ) {
    const distance = this.levenshtein(newEventName, promoterEventName);
    this.logger.warn(
      `Calculated distance ${distance} between names: ${newEventName} - ${promoterEventName}`,
    );
  ...
  private processEventDates(booking: Partial<Booking>): DatePeriod {
    ...
    this.logger.log(
      `Found event start date: ${Util.formatDate(dates.startDate)}${
        dates.endDate ? `, end date: ${Util.formatDate(dates.endDate)}` : ''
      }`,
    );
    return dates;
  }
  ...
  private prepareSignature(eventName: string) {
    ...
    return `${eventName.replace(/ /g, '_').toLocaleLowerCase()}-${now.slice(
      -4,
    )}`;
  }
}
