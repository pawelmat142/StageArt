...
export class ProfileService {
  ...
  public fetchForJwt(uid: string) {
    return this.profileModel.findOne({ uid }).select({
      uid: true,
      name: true,
      roles: true,
  ...
  public async updatePromoterInfoWhenSubmitForm(
    formData: any,
    profile: JwtPayload,
  ) {
  ...
  async createProfile(_profile: Partial<Profile>, registerMode: RegisterMode) {
    const checkName = await this.profileModel
      .findOne({ name: _profile.name })
      .select({ _id: true });
    if (checkName) {
  ...
  async fetchManagerData(uid: string): Promise<ManagerData> {
    const managerProfile = await this.profileModel
      .findOne({ uid })
      .select('managerData');
    if (managerProfile) {
  ...
  async updateArtistProfile(
    ...
    if (!update.modifiedCount) {
      throw new IllegalStateException(
        `Artist profile not modified, uid: ${_profile.uid},`,
      );
    }
