...
export class DataGeneratorService {
  ...
  async dataGenerator() {
    ...
    this.BOOKINGS = await this.bookingsGenerator.generate(
      this.PROMOTERS,
      this.ARTISTS,
    );
  }
  ...
  private async generateManagers() {
    ...
    const manager = await this.generateManager(
      this.MANAGER.name,
      this.MANAGER_EMAIL,
      managerData,
    );
  ...
  private async generateManager(
    ...
    if (managerData) {
      await this.profileService.setManagerData(managerData, {
        uid: user.uid,
      } as JwtPayload);
    }
  ...
  private async generatePromoter(
    name: string,
    email?: string,
  ): Promise<Profile> {
