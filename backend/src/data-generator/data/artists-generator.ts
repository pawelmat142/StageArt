...
export class ArtistsGenerator {
  ...
  private async generateArtists() {
    ...
    const artistProfile2 = await this.generateArtist(
      'Pulse Machine',
      '',
      1,
      [ARTIST_LABEL[2]],
      [ARTIST_STYLE[3], ARTIST_STYLE[2]],
      [ARTIST_MEDIA.instagram, ARTIST_MEDIA.bandcamp, ARTIST_MEDIA.spotify],
    );
    const artistProfile3 = await this.generateArtist(
      'Echo Synthesis',
      '',
      2,
      [ARTIST_LABEL[3], ARTIST_LABEL[4]],
      [ARTIST_STYLE[3], ARTIST_STYLE[4]],
      [
        ARTIST_MEDIA.soundcloud,
        ARTIST_MEDIA.bandcamp,
        ARTIST_MEDIA.instagram,
        ARTIST_MEDIA.you_tube,
      ],
    );
    const artistProfile4 = await this.generateArtist(
      'Synthwave Odyssey',
      '',
      3,
      [ARTIST_LABEL[0]],
      ...
      [
        ARTIST_MEDIA.instagram,
        ARTIST_MEDIA.spotify,
        ARTIST_MEDIA.facebook,
        ARTIST_MEDIA.bandcamp,
      ],
    );

    const artistProfile5 = await this.generateArtist(
      'Infinity Circuit',
      '',
      4,
      [],
    ...
    const artistProfile6 = await this.generateArtist(
      'Arctic Bass',
      '',
      5,
      [ARTIST_LABEL[1]],
    ...
    const artistProfile7 = await this.generateArtist(
      'Quantum Beats',
      '',
      6,
      [ARTIST_LABEL[2]],
      [ARTIST_STYLE[8], ARTIST_STYLE[1], ARTIST_STYLE[7]],
      [ARTIST_MEDIA.instagram, ARTIST_MEDIA.spotify, ARTIST_MEDIA.soundcloud],
    );
    const artistProfile8 = await this.generateArtist(
      'Crystal Oscillator',
      '',
      7,
      [ARTIST_LABEL[3], ARTIST_LABEL[1]],
      [ARTIST_STYLE[4], ARTIST_STYLE[5]],
      [
        ARTIST_MEDIA.instagram,
        ARTIST_MEDIA.spotify,
        ARTIST_MEDIA.facebook,
    ...
    const artistProfile9 = await this.generateArtist(
      'Stellar Waves',
      '',
      8,
      [ARTIST_LABEL[4]],
      [ARTIST_STYLE[8], ARTIST_STYLE[0], ARTIST_STYLE[6], ARTIST_STYLE[7]],
      [ARTIST_MEDIA.instagram, ARTIST_MEDIA.bandcamp, ARTIST_MEDIA.facebook],
    );
  ...
  private async generateArtist(
    ...
    const artist = await this.artistService.createArtist(artistForm, {
      uid: user.uid,
    } as JwtPayload);
    ...
    await this.artistManagerService.setStatus(
      'ACTIVE',
      artist.signature,
      managerProfile,
    );
