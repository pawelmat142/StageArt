...
export class BookingsGenerator {
  ...
  private async getneratePromoterZeroBookings() {
    ...
    for (let i = 0; i < months1.length; i++) {
      ...
      const event = this.getEventInformation(
        `${eventName}`,
        COUNTRIES[6],
        addMonths,
        1,
    ...
    await this.generateBooking(
      promoter,
      this.ARTISTS[1],
      this.getRandomEventInfo(7),
      'DOCUMENTS',
    );
    await this.generateBooking(
      promoter,
      this.ARTISTS[2],
      this.getRandomEventInfo(9),
      'READY',
    );
    ...
    await this.generateBooking(
      promoter,
      this.ARTISTS[3],
      this.getRandomEventInfo(-8),
      'DOCUMENTS',
    );
    await this.generateBooking(
      promoter,
      this.ARTISTS[3],
      this.getRandomEventInfo(-10),
      'READY',
    );
    await this.generateBooking(
      promoter,
      this.ARTISTS[3],
      this.getRandomEventInfo(-22),
      'READY',
    );
  ...
  private async getneratePromoterOneBookings() {
    ...
    await this.generateBooking(
      promoter,
      this.ARTISTS[4],
      this.getRandomEventInfo(-1, -2),
      'READY',
    );
    await this.generateBooking(
      promoter,
      this.ARTISTS[4],
      this.getRandomEventInfo(-2),
      'READY',
    );
    await this.generateBooking(
      promoter,
      this.ARTISTS[4],
      this.getRandomEventInfo(-4, 2),
      'SUBMITTED',
    );
    await this.generateBooking(
      promoter,
    ...
    await this.generateBooking(
      promoter,
      this.ARTISTS[5],
      this.getRandomEventInfo(3),
      'DOCUMENTS',
    );
    await this.generateBooking(
      promoter,
      this.ARTISTS[5],
      this.getRandomEventInfo(5, 1),
      'SUBMITTED',
    );
    await this.generateBooking(
      promoter,
  ...
  private async generateBooking(
    ...
    const eventInformation = event
      ? event
      : this.getRandomEventInfo(3 + 2 * i, 1);
    ...
    const booking = await this.bookingService.submitForm(
      formId,
      Gen.toJwtProfile(promoter),
      { skipValidateDuplicate: true },
    );
  ...
  private promoterInfoFromProfie(
    promoter: Profile,
    index: number,
    companyNameSuffif = 'Sonic Creations',
  ): any {
