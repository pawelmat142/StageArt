...
export class ContractPaperDataProvider {
  ...
  public async prepareData(ctx: BookingContext): Promise<any> {
    ...
    const artistProfile = await this.profileService.findByArtistSignature(
      ctx.artists[0].signature,
    );
    const managerData = await this.profileService.fetchManagerData(
      ctx.booking.managerUid,
    );

    const now = new Date();

    const promoterCountry = this.get(
      formData,
      'promoterInformation.companyCountry.name',
    );
    const promoterAddress = this.get(
      formData,
      'promoterInformation.companyAddress',
    );

    const eventCountry = this.get(
      formData,
      'eventInformation.eventCountry.name',
    );
    ...
    return {
      ...
      promoterName: `${this.get(
        formData,
        'promoterInformation.promoterFirstName',
      )} ${this.get(formData, 'promoterInformation.promoterLastName')}`,
      promoterCompanyName: this.get(
        formData,
        'promoterInformation.companyName',
      ),
