...
export class UploadsService {
  ...
  public async uploadPaperFile(
    ...
    this.logger.log(
      `[START] Upload file ${file.originalname}, Template: ${template}, by: ${profile.uid}, Booking: ${formId}`,
    );
    ...
    if (ctx.booking.promoterUid !== profile.uid) {
      throw new UnauthorizedException(
        `Profile ${profile.uid} has no permission to Booking ${ctx.booking.formId}`,
      );
    }

    const paper = await this.documentService.storeUploadPaper(
      file.buffer,
      ctx,
      template,
      extension,
    ...
    this.logger.log(
      `[STOP] Upload file ${file.originalname}, Template: ${template}, by: ${profile.uid}, Booking: ${formId}`,
    );
    return paper;
  }
  ...
  private validateUploadFile(
    ...
    if (!extension) {
      throw new MessageException(
        `Could not specify extension, mimetype: ${file.mimetype}`,
      );
    }
