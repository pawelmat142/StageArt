...
import {
  ChecklistItem,
  CheklistStep,
} from '../booking/model/checklist.interface';
...
export class ChecklistService {
  ...
  public async refreshChecklist(
    ...
    if (status !== 'DOCUMENTS') {
      this.logger.warn(
        `Skip refresh checklist for booking ${ctx.booking.formId} with status: ${status}`,
      );
      return;
    }

    this.logger.log(
      `Refresh checklist for booking ${ctx.booking.formId} by ${profile.uid}`,
    );

    const papers = await this.documentService.fetchBookingPapers(
      ctx.booking.formId,
    );
    ...
    for (let item of ctx.booking.checklist) {
      const paper: Paper | undefined = papers.find(
        (p) => p.template === item.template,
      );
    ...
    if (updateFlag) {
      ...
      this.logger.log(
        `Checklist without changes for booking ${ctx.booking.formId}`,
      );
    }
  }
  ...
  private markStep(
    ...
    switch (step.type) {
      case 'generate':
        return this.updateStep(item.name, step, !!paper);
      case 'sign':
        return this.updateStep(item.name, step, paper?.status === 'SIGNED');
      case 'verifyAndSign':
        return this.updateStep(item.name, step, paper?.status === 'VERIFIED');
      case 'upload':
        return this.updateStep(item.name, step, paper?.status === 'UPLOADED');
      case 'verify':
        return this.updateStep(item.name, step, paper?.status === 'VERIFIED');
      default:
        return false;
  ...
  private updateStep(
    ...
    if (step.ready && !readyCondition) {
      ...
      this.logger.log(
        `Document ${name}, step [${step.type}] removed ready flag!`,
      );
      return true;
    }
    if (!step.ready && readyCondition) {
