...
import {
  BookingContext,
  SimpleBookingContext,
} from '../booking/model/interfaces';
...
export class DocumentService {
  ...
  public async storeBookingPaper(
    buffer: Buffer,
    ctx: BookingContext,
    template: Template,
  ): Promise<Paper> {
    ...
    await paper.save();
    this.logger.log(
      `Paper saved: ${paper.template} with status: ${paper.status} by ${paper.uid}`,
    );
    return paper;
  }

  public async storeUploadPaper(
    buffer: Buffer,
    ctx: SimpleBookingContext,
    template: Template,
    extension: string,
  ) {
    const paper = new this.paperModel({
  ...
  public async updatePaper(paper: Paper): Promise<Paper> {
    const update = await this.paperModel.updateOne(
      { id: paper.id },
      { $set: paper },
    );
  ...
  public async downloadSignedPaper(
    id: string,
    profile: JwtPayload,
  ): Promise<Paper> {
    const paper = await this.paperModel
  ...
  public async generatePdf(
    ...
    this.logger.log(
      `Generate PDF ${template} for booking ${formId} by ${profile.uid}`,
    );
    const ctx = await this.bookingService.buildContext(formId, profile);
    const data = await this.prepareData(ctx, template);

    const pdfData = await this.getActivePdfDataForArtistAndManager(
      ctx,
      template,
    );
  ...
  private async getActivePdfDataForArtistAndManager(
    ctx: BookingContext,
    template: PdfTemplate,
  ): Promise<PdfData> {
    ...
    if (!pdfData) {
      this.logger.log(
        `Not found active PdfData ${template} for booking ${ctx.booking.formId}`,
      );
  ...
  public async generateSignedPaper(
    paperId: string,
    signatureId: string,
    profile: JwtPayload,
  ): Promise<Paper> {
    ...
    this.logger.log(
      `Generate Paper with signatures ${paper.template} for booking ${ctx.booking.formId} by ${ctx.profile.uid}`,
    );
  ...
  private prepareData(
    ...
    throw new BadRequestException(
      `Not found data provider for template: ${template}`,
    );
  }

  private async updatePaperSignatures(
    paper: Paper,
    signatureId: string,
    ctx: BookingContext,
  ) {
    ...
    if (!bookingRoles.length) {
      throw new UnauthorizedException(
        `Profile ${ctx.profile.uid} has no access to Booking ${ctx.booking.formId} -> Paper ${paper.formId}`,
      );
    }

    const signature = await this.signatureService.fetch(
      signatureId,
      ctx.profile.uid,
    );
    ...
    bookingRoles.forEach((role) => {
      ...
      this.logger.log(
        `Added Signature ${signatureId} of role: ${role} to Paper ${paper.id}`,
      );
    });
