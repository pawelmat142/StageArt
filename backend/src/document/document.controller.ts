...
export class DocumentController {
  ...
  async downloadPaper(
    ...
    PaperUtil.fileResponse(
      res,
      paper.content,
      `${paper.template}.${paper.extension}`,
    );
  }
  ...
  async generate(
    ...
    const paper = await this.documentService.generatePdf(
      formId,
      template,
      profile,
    );
  ...
  async signPaper(
    ...
    const paper = await this.documentService.generateSignedPaper(
      paperId,
      signatureId,
      profile,
    );
    PaperUtil.fileResponse(
      res,
      paper.contentWithSignatures,
      `${paper.template}-signed.${paper.extension}`,
    );
  }
  ...
  @Delete('/:id')
  @UseGuards(JwtGuard)
  deletePaper(@Param('id') id: string, @GetProfile() profile: JwtPayload) {
