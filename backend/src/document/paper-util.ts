...
export abstract class PaperUtil {
  ...
  public static addSignaturesData(
    data: any,
    paperSignatures: PaperSignature[],
  ) {
  ...
  public static resolvePaperStatus(paper: Paper): PaperStatus {
    const promoterSignature = !!paper.signatures?.find(
      (s) => Role.PROMOTER === s.role,
    )?.base64;
    if (promoterSignature) {
      const managerSignature = !!paper.signatures?.find(
        (s) => Role.MANAGER === s.role,
      )?.base64;
      if (managerSignature) {
        return 'VERIFIED';
      }
