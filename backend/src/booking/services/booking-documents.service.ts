...
export class BookingDocumentsService {
  ...
  public async requestDocuments(
    documentsStepRequest: DocumentsStepRequest,
    profile: JwtPayload,
  ): Promise<BookingDto> {
    const ctx = await this.bookingService.buildContext(
      documentsStepRequest.formId,
      profile,
    );
    ...
    this.bookingService.msgToPromoterOrManager(ctx, [
      `Requested documents step for booking at ${BotUtil.formatDate(
        ctx.event.startDate,
      )}`,
    ...
    this.logger.log(
      `Requested documents step for booking ${ctx.booking.formId}, by ${ctx.profile.uid}`,
    );
    return ctx.booking;
  }
}
