...
export class SignatureService {
    ...
    public async listSignatures(uid: string) {
        const signatures = await this.signatureModel.find({ uid, status: { $eq: Status.READY } })
        this.logger.log(`Found ${signatures.length} for ${uid}`)
        return signatures
    }

    public async putSignature(dto: PutSignatureDto, profile: JwtPayload): Promise<{ id: string } | undefined> {
        if (dto.id) {
    ...
    public async cancelSignature(id: string, uid: string) {
        ...
        if (update.modifiedCount) {
            ...
            throw new NotFoundException(`Not found signature ${id} by ${uid} - trying cancell`)
        }
    } 

    private async createSignature(dto: PutSignatureDto, profile: JwtPayload): Promise<Signature> {
    ...
    async fetch(id: string, uid: string): Promise<Signature> {
        ...
        if (!signature) {
            throw new UnauthorizedException(`Not found Signature ${id} for user ${uid}`)
        }
        return signature
    }
