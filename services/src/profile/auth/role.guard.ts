import { ForbiddenException, Injectable, mixin, UnauthorizedException } from "@nestjs/common";
      ...
      class extends RoleGuardBase {
        ...
        override verifyRole() {
            ...
            if (!this.profile.roles.includes(role)) {
                throw new UnauthorizedException(`Role ${role} doesnt match with profile roles: ${this.profile.roles.join(', ')}`)
            }
